...
if Messagedlg('Deseja excluir o registro?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin
   associarCampos;
    ...
    dm.query_movimentacoes.SQL.Add('delete from movimentacoes where id_movimento = :id and tipo = "Saída"');
...
if (edtDescricao.Text <> '') and (edtValor.Text <> '') then
    ...
    MessageDlg('Editado com Sucesso!!', mtInformation, mbOKCancel, 0);
     ...
     dm.query_movimentacoes.Close;
    dm.query_movimentacoes.SQL.Clear;
    dm.query_movimentacoes.SQL.Add('update movimentacoes set valor = :valor where id_movimento = :id and tipo = "Saída"');
...
if (edtDescricao.Text <> '') and (edtValor.Text <> '') then
  ...
  {SALVANDO DADOS NA TABELA DE MOVIMENTAÇÕES}
   dm.query_movimentacoes.Close;
    ...
    dm.query_movimentacoes.ParamByName('tipo').Value := 'Saída';
